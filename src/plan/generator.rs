...
mod tests {
    ...
    mod planning_scope {
        ...
        fn refuses_when_two_collections_share_a_destination_root() {
            ...
            for name in ["Arcade Games", "Game Gear Games"] {
                ...
                let vid = collections::add_version(conn, c, "v1", &format!("/d/{name}.dat"), true)
                    .unwrap();
        ...
        fn allows_a_chd_collection_to_share_a_root_with_a_rom_collection() {
            ...
            let rg =
                dats::create_game(conn, rn, "azumanga", None, None, false, false, false).unwrap();
            ...
            let cg =
                dats::create_game(conn, cn, "azumanga", None, None, false, false, false).unwrap();
        ...
        fn set_filter_restricts_planning_to_requested_sets() {
            ...
            let other =
                generate_plan_filtered(conn, &opts(Some(vec!["TOSEC".to_string()]))).unwrap();
            assert!(
                other.is_empty(),
            ...
            let matched =
                generate_plan_filtered(conn, &opts(Some(vec!["SET".to_string()]))).unwrap();
    ...
    mod duplicate_delete_policy {
        ...
        fn loose_duplicate_is_deleted_canonical_kept_in_place() {
            ...
            let deleted: Vec<_> = plan
                ...
                .iter()
                .filter_map(|op| match &op.kind {
                    OperationKind::Delete { path, reason, .. } => {
        ...
        fn preserve_loose_is_consolidated_into_an_archive_in_the_same_tree() {
            ...
            let game =
                dats::create_game(conn, node, "Game", None, None, false, false, false).unwrap();
            dats::create_rom(
                conn,
                game,
                "game.rom",
    ...
    mod archive_repack_policy {
        ...
        fn shared_content_is_copied_to_each_destination_not_moved() {
            ...
            let g1 =
                dats::create_game(conn, node, "GameA", None, None, false, false, false).unwrap();
            dats::create_rom(conn, g1, "a.rom", 10, Some("BBB"), None, None, "good", None).unwrap();
            let g2 =
                dats::create_game(conn, node, "GameB", None, None, false, false, false).unwrap();
        ...
        fn disk_is_planned_loose_in_a_machine_folder_even_for_a_zip_set() {
            ...
            let g =
                dats::create_game(conn, node, "azumanga", None, None, false, false, false).unwrap();
        ...
        fn complete_container_found_among_many_shared_only_containers() {
            ...
            let g =
                dats::create_game(conn, node, "neoclone", None, None, false, false, false).unwrap();
            dats::create_rom(
                conn,
                g,
                "bios.rom",
