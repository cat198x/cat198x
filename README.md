...
ROMShelf scans your ROM directories, matches every file against
...
```console
$ romshelf status "Nintendo - Game Boy"
Collection Status:
...
ROMShelf separates **knowing** from **changing**:
...
ROMShelf is a single self-contained binary with no runtime dependencies.
...
The full loop is: tell ROMShelf where the reference DATs and your ROMs live,
scan, review status, then plan and apply any reorganisation.

```bash
# 1. Initialise a catalogue in the current directory (~/.romshelf by default)
romshelf init

# 2. Add a reference DAT (or fetch a known one)
romshelf dat add ~/dats/Nintendo\ -\ Game\ Boy.dat
romshelf dat fetch mame            # download a known source; --list to see options

# 3. Point ROMShelf at your ROM directories
romshelf source add /mnt/roms/gameboy

# 4. Scan — hashes every file, resumable, safe to re-run
romshelf scan

# 5. See where you stand
romshelf status "Nintendo - Game Boy" --detailed

# 6. Plan a reorganisation, review it, then apply
romshelf plan
romshelf apply --dry-run           # show exactly what would change
romshelf apply                     # do it (rollback-able)
```

Made a mistake? Undo the most recent apply:

```bash
romshelf apply --rollback
```

## Safety model

ROMShelf is built so that a wrong command costs you time, never data.
...
|---------|--------------|
| `init [path]` | Create a catalogue (data dir, default `~/.romshelf`). |
...
Global flags: `--verbose`, `--quiet`, `--config <file>` (`ROMSHELF_CONFIG`),
`--data-dir <dir>` (`ROMSHELF_DATA_DIR`).

## DAT files

A DAT is an XML database (Logiqx / clrmamepro format) describing a known-good set
of ROMs — each entry's name, size, and hashes. ROMShelf matches your files
...
ROMShelf is part of the 198x family of retro-computing tooling, alongside the
