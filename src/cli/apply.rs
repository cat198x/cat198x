...
use crate::plan::{compute_state_hash, OperationKind, OperationLog, OperationStatus, Plan, SourceRef};
...
fn execute_rollback_move(source: &str, dest: &str, expected_sha1: &str) -> Result<()> {
...
fn execute_repack(sources: &[SourceRef], dest_path: &str, format: &str) -> Result<()> {
    let dest = Path::new(dest_path);
...
fn check_disk_space(plan: &Plan) -> Result<()> {
