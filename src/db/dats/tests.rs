...
fn test_crc_only_dat_entry_is_required_and_matched_by_crc_size() {
    ...
    let reqs = calculate_rom_requirements(conn, version_id, MergeMode::NonMerged, false).unwrap();
...
fn present_keys_matches_rom_present_across_kinds() {
    ...
    crate::db::files::upsert_file(conn, "FULLSHA", Some("HEADERLESS"), None, None, 100).unwrap();
